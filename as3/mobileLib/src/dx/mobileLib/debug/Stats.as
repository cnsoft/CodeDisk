 ...
 *
 *  ----------------------
 *  make it smaller - ADong
 **/

package dx.mobileLib.debug {
	...
	public class Stats extends Sprite {	
		...
		protected var WIDTH : uint; //200
		protected var HEIGHT : uint; //120
		protected const TEXT_HEIGHT:int = 45;
		...
		protected var _currentFps:uint;

		/**
		 * <b>Stats</b> FPS, MS and MEM, all in one.
		 */
		public function Stats(w:int=80, h:int=60) : void {
			WIDTH = w;
			if (h > TEXT_HEIGHT){
				HEIGHT = h;
			}
...
//			xml = <xml><fps>FPS:</fps><ms>MS:</ms><mem>MEM:</mem><memMax>MAX:</memMax></xml>;
			...
			xml = <xml><fps>FPS:</fps><mem>MEM:</mem><memMax>MAX:</memMax></xml>;
		...
		private function init(e : Event) : void {
			...
			graphics.beginFill(colors.bg, .75);
