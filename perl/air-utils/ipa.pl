...
if ($rh_config->{autoinstall}){
    ...
    my $uninstall_cmd = $rh_config->{adt} .' -uninstallApp -platform ios -appid ' .$rh_config->{app_id} .' -device ' .$handler ; 
    my $install_cmd = $rh_config->{adt} .' -installApp -platform ios -package ' .$rh_config->{ipa} .' -device ' .$handler ;  
    print $uninstall_cmd ."\n";
